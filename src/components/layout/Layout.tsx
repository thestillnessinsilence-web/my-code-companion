...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-stone-50 font-sans">
      ...
      {/* Footer */}
      <footer className="bg-[#1a1a1a] text-stone-300">
        {/* Main Footer Content */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8">
            ...
            <div className="lg:col-span-4">
              <Link to="/" className="inline-flex items-center gap-3 mb-6">
                <div className="relative">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#10665c] via-[#7c4d8f] to-[#b695c8] opacity-30" />
                  <svg className="absolute inset-0 w-10 h-10" viewBox="0 0 40 40" fill="none">
                    <polygon points="20,4 36,20 20,36 4,20" stroke="#b695c8" strokeWidth="1" fill="none" opacity="0.6" />
                    <polygon points="20,10 30,20 20,30 10,20" stroke="#b695c8" strokeWidth="0.8" fill="none" opacity="0.4" />
                    <circle cx="20" cy="20" r="4" fill="#b695c8" opacity="0.5" />
                  </svg>
                </div>
                <span className="font-serif text-2xl text-white">Crystal Bloomery</span>
              </Link>
              ...
              <div className="flex items-center gap-4 mt-6">
                <a 
                  ...
                  className="w-10 h-10 rounded-full bg-stone-800 flex items-center justify-center text-stone-400 hover:bg-[#10665c] hover:text-white transition-all duration-300"
            ...
            <div className="lg:col-span-5 grid grid-cols-2 gap-8">
              {/* Explore Column */}
              <div>
                <h4 className="font-serif text-white text-lg mb-5">Explore</h4>
                <nav className="space-y-3">
                  <Link to={createPageUrl('Home')} className="block text-sm text-stone-400 hover:text-white transition-colors">Home</Link>
                  <Link to={createPageUrl('Shop')} className="block text-sm text-stone-400 hover:text-white transition-colors">Shop</Link>
                  <Link to={createPageUrl('About')} className="block text-sm text-stone-400 hover:text-white transition-colors">About</Link>
                  <Link to={createPageUrl('Blog')} className="block text-sm text-stone-400 hover:text-white transition-colors">Blog</Link>
                </nav>
              </div>
              
              {/* Services Column */}
              <div>
                <h4 className="font-serif text-white text-lg mb-5">Services</h4>
                <nav className="space-y-3">
                  <Link to={createPageUrl('BookReading')} className="block text-sm text-stone-400 hover:text-white transition-colors">Book a Reading</Link>
                  <Link to={createPageUrl('Wholesale')} className="block text-sm text-stone-400 hover:text-white transition-colors">Wholesale</Link>
                  <Link to={createPageUrl('Events')} className="block text-sm text-stone-400 hover:text-white transition-colors">Events</Link>
                  <Link to={createPageUrl('Contact')} className="block text-sm text-stone-400 hover:text-white transition-colors">Contact</Link>
                </nav>
              </div>
            </div>
            ...
            <div className="lg:col-span-3">
              <h4 className="font-serif text-white text-lg mb-5">Stay Connected</h4>
              ...
              <a 
                ...
                className="inline-block mt-4 text-sm text-[#b695c8] hover:text-white transition-colors"
              >
        ...
        <div className="border-t border-stone-800">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <span className="text-xs text-stone-500">© {new Date().getFullYear()} Crystal Bloomery. All rights reserved.</span>
